...
# do wypisywania danych napisać oddzielną metodę
# dodać kolorki
...
def download_file(url, filename):
    ...
    progressbar = progbar.ProgressBar(filesize + size, 50)
    clear = progbar.ClearScreen()
    ...
    while True:
        ...
        current_speed = current_download / current_time
        # w przybliżeniu ile jeszcze sekund pobierania przed nami
        eta = (filesize - total_read) / 1024 / current_speed
        clear.two()
        if bytes_read == 0:
            progressbar.update(filesize + size)
            sys.stdout.write("%s  %d kb  %d kb/s  eta %d" % (progressbar, \
    ...
    # 2
    clear.one()
    for i in xrange(0, filesize + 1):
        bytes = urlfile.read(step)
        outfile.write(bytes)
        bytes_read = len(bytes)
        total_read += bytes_read
        if bytes_read == 0:
            print("Powinno być 100%")
        ...
        current_time = time.time() - start_time
        # aktualna prędkość pobierania
        current_speed = total_read / 1024 / current_time
        # w przybliżeniu ile jeszcze sekund pobierania przed nami
        eta = (filesize - total_read) / 1024 / current_speed
        sys.stdout.write("%s  %d kb  %d kb/s  eta %d" % (progressbar, \
                current_download, current_speed, eta))
